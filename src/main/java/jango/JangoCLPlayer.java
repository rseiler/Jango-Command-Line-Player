...
public class JangoCLPlayer {
	...
	private static Pattern idPattern = Pattern.compile("(\\d+)\\.mp3");
	private static Pattern songPattern = Pattern.compile("song\":\"(.*?)\"");
	private static Pattern artistPattern = Pattern.compile("artist\":\"(.*?)\"");
	private static Pattern stationIdPattern = Pattern
			.compile("/stations/(\\d+)/tunein.*?class=\"sp_tgname\">([\\w\\d /]+)</span");
	...
	public static void main(String[] args) throws ClientProtocolException, IOException {
		...
		EntityUtils.consume(client.execute(new HttpGet("http://www.jango.com/stations/" + stationId + "/tunein"))
				.getEntity());
